...
public interface ICategoryRepository
{
    ...
    Task<Category?> GetByIdAsync(CategoryId id, StoreId storeId, CancellationToken ct = default);

    /// <summary>Kiểm tra tên danh mục đã tồn tại chưa.</summary>
    Task<bool> ExistsByNameAsync(string name, StoreId storeId, CancellationToken ct = default);

    /// <summary>Kiểm tra tên danh mục đã tồn tại chưa (trừ danh mục đang sửa).</summary>
    Task<bool> ExistsByNameAsync(string name, CategoryId excludeId, StoreId storeId, CancellationToken ct = default);
